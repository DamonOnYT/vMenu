...
namespace vMenuClient
{
    public class NeonMenu : BaseScript
    {
        ...
        {
            Neons = new Menu($"Neon Menu", $"Neon customization for cars");
            ...
            string AwaitDelayDyn(MenuDynamicListItem item, bool left)
            {
                if (left)
                {
                    ...
                    if (newvalue < 0)
                    {
                        ...
                        {
                            Notify.Error($"AlsekRGB: Min value allowed for ~b~Rate~w~ is 0");
                        }
                    }
                    else
                    {
            ...
            //Adding the neon changer sub menu
            {
                ...
                MenuItem NeonButton = new MenuItem($"Neon Color Picker", $"eon fine tuning !!THESE DO NOT TAKE EFFECT UNLESS YOU CYCLE ONE OF THE NEON MODES!!")
                {
                    Label = "→→→"
                };
            ...
            //Adding the neon custom sub menu
            {
                ...
                MenuItem NeonCustomButton = new MenuItem($"Custom Neon Mode", $"A neon mode you can configure")
                {
                    Label = "→→→"
                };
        ...
        {
            string result = await GetUserInput(windowTitle: "Enter a number between 0 and 1000", maxInputLength: 4);
            ...
            if (!string.IsNullOrEmpty(result))
            {
                ...
                if (resultint > 1000)
                {
                    if (Type == 1)
                    {
                        Notify.Error($"AlsekRGB: Max value allowed for ~b~Rate~w~ is 1000");
                    }
                }
                else
                {
